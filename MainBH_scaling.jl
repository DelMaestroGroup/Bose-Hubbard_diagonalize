...
println("size(basis) = ",Base.summarysize(basis)/1024^3," gb")########################################

println("==================================(end.)==================================")########################################

println("======================(Generating the Cycles)======================")########################################

    @time Cycles_leaders, CycleSize, NumOfCycles, InvCycles_Id = Translational_Symmetry_Reflection_Cycles_gs(basis)
println("size(Cycles_leaders) = ",Base.summarysize(Cycles_leaders)/1024^3," gb")########################################
println("size(InvCycles_Id) = ",Base.summarysize(InvCycles_Id)/1024^3," gb")########################################
...
println("==================================(end.)==================================")########################################
    ...
    Eg=0.0
    for U in U_range
println("======================(Generating the Hamiltonian)======================")########################################
        @time H = sparse_Block_Diagonal_Hamiltonian_q0R1_gs(basis, Cycles_leaders, CycleSize, NumOfCycles, InvCycles_Id, T, U)
println("size(H) = ",Base.summarysize(H)/1024^3," gb")########################################
println("==================================(end.)==================================")########################################
println("======================(Diagonalizing the Hamiltonian)======================")########################################

        @time E,d = eigs(H,nev=1,which=:SR,tol=1e-13,v0=Ψ)
println("==================================(end.)==================================")########################################
        H=Nothing
        GC.gc()
        Eg=Real(E[1])
...
println("======================(Calculating The entanglement)======================")########################################
           @time S1_sp,S1_acc,S2_sp,S2_acc,Neg,Neg_acc,Pn,S1,S2,SNeg  = spatial_entropy_op_Ts_gs_scaling(basis,A,d, CycleSize,InvCycles_Id)
println("==================================(end.)==================================")########################################
