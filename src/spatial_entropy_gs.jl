"""
	spatial_entropy_acc_gs(basis::AbstractIntbasis, Asize::Int, d::Vector{T}, CycleSize::Array{Int64,1}, InvCycles_Id::Vector{Int64}, getspectrum::Bool, U::Float64) where {T<:Number}

Calculate both the spatial and the accessible entanglement entropies of a
...
"""
function spatial_entropy_acc_gs(basis::AbstractIntbasis, Asize::Int, d::Vector{T}, CycleSize::Array{Int64,1}, InvCycles_Id::Vector{Int64}, getspectrum::Bool, U::Float64) where {T<:Number}

	L=basis.L
	N=basis.N
	M=L+N
	A,B = 1:Asize, Asize+1:L
	...
	for n in norms
        ...
        S2_acc=-2.0*log(sum(S2.^(0.5)))
        Neg_acc=log(sum(SNeg.^(2.0)))


	S1_sp,S1_acc,S2_sp,S2_acc,Neg,Neg_acc 
end
...
Spatial entropy using symmetries with S2_sp and Neg.
"""
function spatial_entropy_acc_gs_Rest(basis::AbstractIntbasis, Asize::Int, d::Vector{T}, CycleSize::Array{Int64,1}, InvCycles_Id::Vector{Int64}) where {T<:Number}
